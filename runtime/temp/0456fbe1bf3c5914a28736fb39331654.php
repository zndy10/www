<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:41:"./application/admin/view/goods\_spec.html";i:1522317281;s:61:"D:\phpStudy\WWW\www\application\admin\view\public\layout.html";i:1522317281;}*/ ?>
<!doctype html>
<html>
<head>
  ...
  <div class="fixed-bar">
    ...
    <div class="ncap-form-default">
      <dl class="row">
        ...
        <dd class="opt">
          <input type="text" value="<?php echo $spec['name']; ?>" name="name"   class="input-txt"/>
      ...
      <dl class="row">
        <dt class="tit">
          <label for="parent_id">所属商品模型：</label>
        </dt>
        <dd class="opt">                 
            <select name="type_id" id="type_id" class="small form-control">
                ...
                <?php if(is_array($goodsTypeList) || $goodsTypeList instanceof \think\Collection || $goodsTypeList instanceof \think\Paginator): $i = 0; $__LIST__ = $goodsTypeList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                 <option value="<?php echo $vo[id]; ?>" <?php if($vo[id] == $spec[type_id]): ?>selected="selected"<?php endif; ?>><?php echo $vo[name]; ?></option>
	   ...
	   <dl class="row" style="display:none;">
        ...
        <dd class="opt">
            <textarea rows="5" cols="30" name="items" class="input-txt" style=" height:100px;"/><?php echo $spec['items']; ?></textarea>
            <p class="notic">一行为一个规格项，多个规格项用换行输入</p>
      ...
      <dl class="row">
        ...
        <dd class="opt">
            <input type="text" value="<?php echo (isset($spec['order']) && ($spec['order'] !== '')?$spec['order']:'50'); ?>" name="order"  class="input-txt"/>
            <span id="err_order" class="err" style="color:#F00; display:none;"></span>           
          
        </dd>
      </dl>	                
      <div class="bot"><a href="JavaScript:void(0);" onClick="ajax_submit_form('addEditSpecForm','<?php echo U('Goods/addEditSpec?is_ajax=1'); ?>');" class="ncap-btn-big ncap-btn-green" id="submitBtn">确认提交</a></div>
    </div>
