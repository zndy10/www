<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:42:"./application/admin/view/goods\_goods.html";i:1522317281;s:61:"D:\phpStudy\WWW\www\application\admin\view\public\layout.html";i:1522317281;}*/ ?>
<!doctype html>
<html>
<head>
...
<!--通用信息-->
        <div class="ncap-form-default tab_div_1">
            ...
            <dl class="row">
                ...
                <dd class="opt">
                      ...
                      <select name="cat_id_2" id="cat_id_2" onChange="get_category(this.value,'cat_id_3','0');getCategoryBindList(this.value)" onclick="get_category(this.value,'cat_id_3','0');getCategoryBindList(this.value)" class="small form-control">
                        <option value="0">请选择商品分类</option>
                      </select>
            ...
            <dl class="row">
                ...
                <div class="input-file-show">
            <span class="type-file-box">
                <input type="text" id="videotext" name="video" value="<?php echo $goodsInfo['video']; ?>" class="type-file-text">
                <span id="video_button">
                    ...
                    <?php else: ?>
                        ...
                        <input class="type-file-file" onClick="GetUploadify(1,'','goods','video_call_back','Flash')" size="30" hidefocus="true" nc_type="change_site_logo" title="点击按钮选择文件并提交表单后上传生效">
                    <?php endif; ?>
                </span>
            </span>
                    </div>
            ...
            <dl class="row">
                ...
                <dd class="opt">
                    <input type="text" value="<?php echo $goodsInfo['volume']; ?>" name="volume" class="t_mane" onKeyUp="this.value=this.value.replace(/[^\d.]/g,'')" onpaste="this.value=this.value.replace(/[^\d.]/g,'')" />
                    <span class="err" id="err_volume" style="color:#F00; display:none;"></span>
        ...
        <dd class="opt">
          <div class="onoff">
            <label id="is_free_shipping_label_1" for="is_free_shipping1" class="cb-enable <?php if($goodsInfo[is_free_shipping] == 1): ?>selected<?php endif; ?>">是</label>
            <label id="is_free_shipping_label_0" for="is_free_shipping0" class="cb-disable <?php if($goodsInfo[is_free_shipping] == 0): ?>selected<?php endif; ?>">否</label>
            <input id="is_free_shipping1" class="is_free_shipping" name="is_free_shipping" value="1" type="radio" <?php if($goodsInfo[is_free_shipping] == 1): ?> checked="checked"<?php endif; ?>>
            <input id="is_free_shipping0" class="is_free_shipping" name="is_free_shipping" value="0" type="radio" <?php if($goodsInfo[is_free_shipping] == 0): ?> checked="checked"<?php endif; ?>>
          ...
          <p class="notic"></p>
            ...
            <dl class="row">
                ...
                <dd class="opt">
                    <?php if($goodsInfo['goods_id'] > 0): if($goodsInfo['is_virtual'] == 1): ?>
                            <span>是</span>
                        ...
                        <?php endif; else: ?>
                    <div class="onoff">
                        <label for="is_virtual1" class="cb-enable <?php if($goodsInfo[is_virtual] == 1): ?>selected<?php endif; ?>">是</label>
                        <label for="is_virtual0" class="cb-disable <?php if($goodsInfo[is_virtual] == 0): ?>selected<?php endif; ?>">否</label>
                        <input class="is_virtual" id="is_virtual1" name="is_virtual" value="1" type="radio" <?php if($goodsInfo[is_virtual] == 1): ?> checked="checked"<?php endif; ?>>
                        <input class="is_virtual" id="is_virtual0" name="is_virtual" value="0" type="radio" <?php if($goodsInfo[is_virtual] == 0): ?> checked="checked"<?php endif; ?>>
                    </div>
                    <?php endif; ?>
                    <p class="notic"></p>
                </dd>
            ...
            <dl class="row virtual" style="display: none;">
                ...
                <dd class="opt virtual">
                    <input id="virtual_indate" name="virtual_indate" value="<?php echo date('Y-m-d',$goodsInfo[virtual_indate]); ?>" class="input-txt" type="text">
                    <span class="err" id="err_virtual_indate" style="color:#F00; display:none;"></span>
            ...
            <dl class="row virtual" style="display: none;">
                ...
                <dd class="opt">
                    <input type="text" value="<?php echo $goodsInfo['virtual_limit']; ?>" class="t_mane" name="virtual_limit" onKeyUp="this.value=this.value.replace(/[^\d.]/g,'')" onpaste="this.value=this.value.replace(/[^\d.]/g,'')" onblur="checkInputNum(this.name,1,10,'',1);" />
                    <span class="err" id="err_virtual_limit" style="color:#F00; display:none;"></span>
...
<!--积分折扣-->
		<div class="ncap-form-default tab_div_5" style="display:none;">
            ...
            <dl class="row">
                ...
                <dd class="opt">
                    <input type="text" value="<?php echo $goodsInfo['exchange_integral']; ?>" name="exchange_integral" class="t_mane" onKeyUp="this.value=this.value.replace(/[^\d.]/g,'')" onpaste="this.value=this.value.replace(/[^\d.]/g,'')" />	不得高于商品最低价格与兑换比的积，如果设置0，则不支持积分抵扣
...
$(document).ready(function(){
    ...
    function getCategoryBindList(val){
        $.ajax({
            'url':"<?php echo U('goods/getCategoryBindList'); ?>",
            'data':{cart_id:val},
            'dataType':'json',
            success:function(data){
                if(data.status == 1){
